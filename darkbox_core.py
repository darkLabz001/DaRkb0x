...
PAYLOAD_REQUEST_PATH = "/dev/shm/db_payload_request.json"   # WebUI uses db_ prefix
...
def _display_loop():
    _FRAME_PATH     = os.environ.get("DB_FRAME_PATH", "/dev/shm/ktox_last.jpg")
    _FRAME_ENABLED  = os.environ.get("DB_FRAME_MIRROR", "1") != "0"
    _FRAME_INTERVAL = 1.0 / max(1.0, float(os.environ.get("DB_FRAME_FPS", "10")))
    last_save = 0.0
...
def getButton(timeout=120):
    ...
    Checks WebUI virtual buttons (Unix socket via db_input) first.
...
# These map menu label text → FA glyph so every item gets an icon like DaRkb0x.
...
def boot():
    ...
    if not os.path.exists(db_loot):
        try: os.symlink(LOOT_DIR, db_loot)
        except OSError: pass
