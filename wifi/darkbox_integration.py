#!/usr/bin/env python3
"""
DaRkb0x WiFi Integration Functions
===================================
Integration functions to add WiFi support to DaRkb0x's main system
...
Usage in darkbox.py:
    from wifi.darkbox_integration import get_best_interface, get_interface_ip
...
BOOT_LOG_FILE = "/root/DaRkb0x/loot/network/interface_boot_log.txt"
...
def get_best_interface(prefer_wifi=False, bypass_checks=False):
    ...
    if user_preference:
        ...
        if status["connected"] and status["ip"]:
            ...
            try:
                import os
                pref_file = "/root/DaRkb0x/wifi/interface_preferences.json"
...
def show_interface_info():
    ...
    print("\n" + "="*50)
    print("DaRkb0x Network Interface Status")
    print("="*50)
...
# Integration functions for specific DaRkb0x features
...
def save_interface_preference(tool, interface):
    ...
    config_file = "/root/DaRkb0x/wifi/interface_preferences.json"
...
def backup_routing_config():
    ...
    backup_file = "/root/DaRkb0x/wifi/routing_backup.json"
...
def update_dns_for_interface(interface):
    ...
    try:
        ...
        if dns_servers:
            ...
            resolv_content = "# Generated by DaRkb0x WiFi Integration\n"
...
def force_interface_as_default(interface):
    ...
    try:
        ...
        try:
            with open('/etc/resolv.conf', 'w') as f:
                f.write(f"# DaRkb0x forced DNS for {interface} - {datetime.now()}\n")
...
def set_darkbox_interface(interface, lcd_callback=None):
    """Explicitly set which interface DaRkb0x tools should use - IMMEDIATE."""
    def lcd_update(msg):
        """Send short message to LCD if callback provided."""
        if lcd_callback:
            lcd_callback(msg)
    
    print(f"🔄 IMMEDIATELY setting DaRkb0x to use: {interface}")
    ...
    if success:
        ...
        print(f"✅ DaRkb0x IMMEDIATELY switched to {interface}")
        print(f"   IP: {interface_ip}")
        ...
        print(f"❌ FAILED to switch DaRkb0x to {interface}")
        lcd_update("FAILED!")
        return False
...
def switch_wifi_interface(from_interface, to_interface):
    ...
    success = set_darkbox_interface(to_interface)
...
def get_current_darkbox_interface():
    """Get the interface currently being used by DaRkb0x."""
...
def list_wifi_interfaces_with_status():
    ...
    current_iface = get_current_darkbox_interface()
...
def main():
    ...
    print("DaRkb0x WiFi Integration Test")
    print("="*40)
